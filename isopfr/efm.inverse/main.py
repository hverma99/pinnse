...
def main():
    ...
    # ------------------- Inverse PINN Setup ----------------------------
    # -------------- Unknown Activation Energy (E) ----------------------
    E_true = np.array([105.2, 124.89, 89.66], dtype=np.float32)  # kJ/mol
    # E = theta * 1e3, theta in kJ/mol is trainable parameter
    theta = nn.Parameter(
    ...
    norm_I_S_data, norm_D_S_data, I_S_metrics, D_S_metrics = Normalization.min_max_pfr(
        I_S_data=I_S_data, D_S_data=D_S_data, formulation="EFM", species=species
    )

    physics = Physics(I_S_metrics, D_S_metrics, species, nu, key_species, k, theta)
    ...
    loss_fn = nn.MSELoss()
    optimizer = torch.optim.Adam(list(model.parameters()) + [theta], lr=5e-4)
