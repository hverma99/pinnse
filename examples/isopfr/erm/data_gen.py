...
"""
Generate labeled input-output (I_S/D_S) datasets for the isothermal PFR process (ERM).
...
def D_S_samples(I_S: pd.DataFrame):
    ...
    D_S : pd.DataFrame
        ...
        - E_j   : extent of reaction for reaction j

    Notes
    -----
    - For each sampled input condition, the extent of reaction-based formulation (ERM)
      of the isothermal PFR model is solved using the specified kinetic and
      process parameters.
    - The model solution provides the extent of reactions at the
      specified reactor volume.
    - The collected extent of reactions form the labeled output dataset `D_S`.
    """
    ...
    for i in tqdm(range(N_D)):
        ...
        sol = model.solve(V)
        E_exit = sol.y[:, -1]
        Y[i, :] = E_exit.astype(np.float32, copy=False)
    D_S = pd.DataFrame(Y, columns=[f"E_R{j+1}" for j in range(nu.shape[0])])
    return D_S
...
if __name__ == "__main__":
    ...
    D_S_data = D_S[[*(f"E_R{j+1}" for j in range(nu.shape[0]))]]
