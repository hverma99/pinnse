...
def truth_eval(
    ...
    """
    Evaluate the first-principles CM model along the reactor profile.
    """
    model = CM(species, nu, key_species, k, E, F_in, P, T)
    sol = model.solve(V_end=float(V_eval[-1]), V_eval=V_eval)
    X_truth = sol.y.T.astype(np.float32)

    return pd.DataFrame(X_truth, columns=[f"X_R{j+1}" for j in range(nu.shape[0])])
...
def plot_conversion_profile(
    ...
    """
    Plot truth vs PINN reaction-conversion profiles.
    """
    fig, ax = plt.subplots(figsize=(8, 6), dpi=300)

    reactions = ["X_R1", "X_R2", "X_R3"]
    label_reactions = [r"$X_{R1}$", r"$X_{R2}$", r"$X_{R3}$"]
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, "conversion_profile.png"), bbox_inches="tight")
    plt.close()
...
if __name__ == "__main__":
    ...
    plot_conversion_profile(V_eval, D_S_truth, D_S_pinn, save_dir)
