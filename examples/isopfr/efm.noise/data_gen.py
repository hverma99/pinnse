...
"""
Generate labeled input-output (I_S/D_S) datasets for the isothermal PFR process (EFM).
...
def D_S_samples(I_S: pd.DataFrame):
    ...
    D_S : pd.DataFrame
        ...
        - F_out_i   : effluent molar flowrates for each of components

    Notes
    -----
    - For each sampled input condition, the effluent-flow formulation (EFM)
      of the isothermal PFR model is solved using the specified kinetic and
      process parameters.
    - The model solution provides the outlet species molar flowrates at the
      specified reactor volume.
    - The collected outlet flowrates form the labeled output dataset `D_S`.
    """
    ...
    Y = np.zeros((N_D, N_species), dtype=np.float32)
    ...
    for i in tqdm(range(N_D)):
        ...
        F_out = model.solve(V)
        Y[i, :] = F_out
    D_S = pd.DataFrame(Y, columns=[f"F_ot_{sp}" for sp in species])
    return D_S
...
if __name__ == "__main__":
    ...
    D_S_data = D_S[[*(f"F_ot_{sp}" for sp in species)]]
