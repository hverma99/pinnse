...
from utils import Normalization, Denormalization, Analyze
...
def plot_profiles(
    ...
    Q_a = D_S_aspen["Q"].to_numpy().reshape(nP, nT)
    Q_p = D_S_pinn["Q"].to_numpy().reshape(nP, nT)
    ...
    if P_target is not None:
        ...
        # ------------------ combined X + Q plot versus T_flash ------------------
        fig, ax1 = plt.subplots(figsize=(8, 6), dpi=300)
        ax2 = ax1.twinx()

        # X components on left axis
        for comp, color in zip(species, colors):
            k = species.index(comp)
            ax1.plot(
                T,
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, f"X_Q_vs_T_at_P_{P[i]:.4g}.png"))
        plt.close()
    ...
    if T_target is not None:
        ...
        # ------------------ combined X + Q plot versus P_flash ------------------
        fig, ax1 = plt.subplots(figsize=(8, 6), dpi=300)
        ax2 = ax1.twinx()
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, f"X_Q_vs_P_at_T_{T[j]:.4g}.png"))
        plt.close()
...
if __name__ == "__main__":
    ...
    _, D_S_metrics = Normalization.min_max(D_S_data, normalize_cols=["Q"])
